...
class OrganizationalChart(models.Model):
    ...
    def get_employee_data(self, emp_id):
        ...
        data = {
            ...
            'img': parent_emp.image_1920,
            'title': self._get_position(parent_emp),
            'children': [],
            'office': self._get_image(parent_emp),
        }
        employees = self.env['hr.employee'].sudo().sudo().search([('parent_id', '=', parent_emp.id)])
    ...
    def get_children(self, emp, style=False):
        data = []
        emp_data = {'id': emp.id, 'name': emp.name, 'img': emp.image_1920, 'title': self._get_position(emp), 'office': self._get_image(emp)}
        childrens = self.env['hr.employee'].sudo().search([('parent_id', '=', emp.id)])
        for child in childrens:
            sub_child = self.env['hr.employee'].sudo().search([('parent_id', '=', child.id)])
            next_style = self._get_style(style)
            if not sub_child:
                data.append({'id': child.id, 'name': child.name, 'img': child.image_1920, 'title': self._get_position(child), 'className': next_style,
