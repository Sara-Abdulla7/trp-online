...
class InsPartnerLedger(models.TransientModel):
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if self.type:
                account_type = tuple([self.type, 'none'])

            WHERE += ' AND a.account_type IN %s' % str(account_type)
    ...
    def process_data(self):
        ...
        for partner in partner_ids:
            ...
            for row in cr.dictfetchall():
                if (data.get('display_accounts') == 'balance_not_zero' and currency.is_zero(row['debit'] - row['credit']))\
                    ...
                    int_credit+=row['credit']
                    int_debit+=row['debit']
                    int_balance+=row['balance']
        ...
        subtotal = {'SUBTOTAL': {
            'name': 'Total',
            'initial_credit': company_currency_id.round(int_credit),
            'initial_debit': company_currency_id.round(int_debit),
            'symbol': symbol,
            'position': position,
            'initial_balance': company_currency_id.round(int_balance)
        }}

        return move_lines,subtotal
    ...
    def get_report_datas(self, default_filters={}):
        ...
        if self.validate_data():
            ...
            account_lines,subtotal = self.process_data()
            print(account_lines)
            return filters, account_lines ,subtotal

    def action_pdf(self):
        filters, account_lines,subtotal = self.get_report_datas()
        return self.env.ref(
    ...
    def action_xlsx(self):
        ...
        filter, account_lines,subtotal = record.get_report_datas()
        # Formats
        ############################################################
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 1, sub_line.get('lcode'),
                                                    line_header_light)
                            account_name = ''
                            if sub_line.get('account_name'):
                                # account_name = sub_line['account_name']['en_US']
                                account_name = sub_line['account_name']
                            sheet.write(row_pos, 2, account_name or '',
                                                    line_header_light)
        ...
        # Sub total line
        row_pos += 2
        sheet.write(row_pos, 1,subtotal['SUBTOTAL'].get('name'),
        ...
        sheet.write(row_pos, 5, float(
            subtotal['SUBTOTAL'].get('initial_debit')),
                    line_header_light_total)
        sheet.write(row_pos, 6, float(
            subtotal['SUBTOTAL'].get('initial_credit')),
                    line_header_light_total)
        sheet.write(row_pos, 7, float(
            subtotal['SUBTOTAL'].get('initial_balance')),
                    line_header_total)
        # Close and return
        #################################################################
        workbook.close()
